...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!updateLink || !z.url().safeParse(updateLink).success)
      NextResponse.json(
        { success: false, message: 'Failed to create Stripe Account Link' },
        { status: 404 }
      );

    return NextResponse.json(
      { success: true, message: 'Stripe Account Link Created Successfully', data: { updateLink } },
      { status: 201 }
    );
  } catch (error) {
    console.error('[Stripe] Account update link error:', error);
    return NextResponse.json({ error: 'Failed to generate account update link' }, { status: 500 });
  }
}
